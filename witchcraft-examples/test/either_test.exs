...
#   def decode({:json, body}), do: body |> Jason.decodeode!() |> Right.new()
#   def decode({mime, _body}), do: :unknown_mime |> Left.new()

#   describe "Monad" do
#     test "limitation of Apply" do
#       # Right, ok duplicated => should be %Right{right: %{}}
#       assert (&decodeode/2) <~ Right.new(:json) <<~ Right.new("{}") == %Right{right: {:ok, %{}}}
#       # Right, error is non-sense => should be %Left{left: :error}
#       assert (&decodeode/2) <~ Right.new(:jpg) <<~ Right.new(<<1>>) == %Right{right: :error}
#     end
...
#     def assert_inventory_suffice(inventory, required) when required <= inventory, do: Right.new()
#     def assert_inventory_suffice(inventory, required), do: Left.new(:out)
